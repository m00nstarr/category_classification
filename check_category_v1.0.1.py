...
# Goal :
# make standard maincategory using each row's subcategory

# standard was posted in gdrive/2020/readme
# code last update : 2020 / 07/ 14

# How to use
# > python3.5 check_category.py
# > (File_name).xlsx   (insert file_name)

# input excel file must have blank (Ith) column name: 'Maincategory'
# input file must be located in same directory of code
# example of input excel file )
# Country | type | OS | year ..... |Maincategory (Ith column)| subcategory |......
# KR      | FREE |AND | 2020 ......|         (blank)         | Social      |......
...
print()
print("** Read "+ str(KR_2020.shape[0]) + " records of "+ file_name+ ".xlsx") ## 20/07/14 추가 
...
# 생성일자 추가하여 output filename 수정 : 20/07/14
...
print("** copying original to outputfile completed") ## 20/07/14 추가

null_count=0
for i in contents[contents['subcategory'].isnull()].index:
  null_edit_set.add(contents['App'][i])
  null_count = null_count +1 ## 20/07/14 추가

# null count세서 모두 채웠는지 확인함. 20/07/14
...
for i in range(len(contents)):
  ...
  if sub_name == "Sports":
    ...
    or sub_name == "Auto & Vehicles" or sub_name == "Beauty":
  ...
  elif sub_name == "Art & Design" or sub_name == "Personalization":
    new_worksheet.cell(row=i + 2, column=column_index_from_string('i')).value = "Productivity"
  ...
  elif sub_name == "Educational":
    new_worksheet.cell(row=i + 2, column=column_index_from_string('i')).value = "Kids"
...
# mark is added part ( code about describing process of program )
print() #
